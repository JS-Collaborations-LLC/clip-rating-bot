...
async function saveClip(clipData, interactionId, messageId) {
  try {
...
async function getClipByInteractionId(interactionId) {
  try {
    return await Clip.findOne({ interactionId });
  } catch (error) {
    console.error('Error retrieving clip by interaction ID:', error);
    throw error;
  }
}

async function getClipByMessageId(messageId) {
  try {
    return await Clip.findOne({ messageId });
  } catch (error) {
    console.error('Error retrieving clip by message ID:', error);
    throw error;
  }
}

module.exports = { connectDB, db, saveClip, getClipByInteractionId, getClipByMessageId };
